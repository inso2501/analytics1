...
women
plot(women)
plot(women, type='p', pch=05, col='red')
plot(women, type='l')
plot(women, type='b')
...
plot(women)
abline(lm(women$weight ~ women$height), col='red', lty=5, lwd=1)
...
hist(women$height, breaks=10, col=10:25)

#histogram2
(x = rnorm(100,50,10))
hist(x)

hist(x, freq=F, col=1:25)
lines(density(x))
...
barplot(mtcars$mpg,col = 1:32)
hist(mtcars$hp,col = 1:36,breaks = 32)
cor(mtcars$am,mtcars$gear)
cov(mtcars$am,mtcars$gear)
head(mtcars)
