...
#nn: display node no.
#cex:amplify the graph
#extra:
printcp(fit) #select complexity parameter
prunetree2 = prune(fit, cp=.014)#xerror is lowest
...
ndata2= data.frame(pclass=3,sex=factor(c('male')),sibsp=0,age=21)
predict(prunetree2, newdata=ndata2, type='class')
predict(prunetree2, newdata=ndata2, type='prob')
