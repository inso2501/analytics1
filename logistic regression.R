...
test_set2
(prob_pred2 = predict(logitmodel2, type = 'response', newdata = test_set2))#will get result even if we dont type response
...
?optimalCutoff
(optCutOff <- optimalCutoff(dataset$purchased, prob)[1] ) 
confusionMatrix(dataset$purchased, prob, threshold = optCutOff)

y_pred = ifelse(prob_pred > 0.73, 1, 0)
head(cbind(test_set$purchased, y_pred),10)

# Making the Confusion Matrix
cm = table(test_set[,5], y_pred)
cm
(accuracy = ( 61 + 18)/ sum(cm))
